...
FONT_STYLE="${FONT_STYLE:-big}"          # "big" = HIDDENEYE tarzı kalın 2 renkli, "classic" = eski ince tek renk
FONT_SCALE="${FONT_SCALE:-2}"            # kalınlık/boyut çarpanı
FONT_COLOR1="${FONT_COLOR1:-\033[1;36m}" # ilk kelime rengi (varsayılan: cyan)
FONT_COLOR2="${FONT_COLOR2:-\033[1;31m}" # ikinci kelime/kalan renk (varsayılan: kırmızı)
...
if [ "$FONT_STYLE" = "big" ]; then
    render_big_text "$BANNER_TEXT" "$FONT_SCALE" "$FONT_COLOR1" "$FONT_COLOR2"
else
